...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not name and not increment or not start:
            print("Name, increment or start is missing.")
            return

        try:
            with connection.cursor() as cursor:
                cursor.execute(f"CREATE SEQUENCE {name} INCREMENT {increment} START {start};")
                print(f"Database sequence has been created successfully (name: {name}).")
        except ProgrammingError as exception:
            print(f"Database sequence could not be created (reason: {str(exception)}).")
